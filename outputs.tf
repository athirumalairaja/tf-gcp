...
output "ssh_commands" {
  ...
  value = merge(
    {
      for i, instance in google_compute_instance.jumpbox :
      instance.name => "ssh -i ${var.ssh_private_key_path} ${var.ssh_username}@${instance.network_interface[0].access_config[0].nat_ip}"
    },
    {
      for i, instance in google_compute_instance.server :
      instance.name => "ssh -i ${var.ssh_private_key_path} ${var.ssh_username}@${instance.network_interface[0].access_config[0].nat_ip}"
    },
    {
      for i, instance in google_compute_instance.worker :
...
output "cluster_summary" {
  ...
  value = {
    ...
    server = length(google_compute_instance.server)
    workers     = length(google_compute_instance.worker)
    total_nodes = length(google_compute_instance.jumpbox) + length(google_compute_instance.server) + length(google_compute_instance.worker)
...
output "quick_ssh_jumpbox" {
  ...
  value = length(google_compute_instance.jumpbox) > 0 ? "ssh -i ${var.ssh_private_key_path} ${var.ssh_username}@${google_compute_instance.jumpbox[0].network_interface[0].access_config[0].nat_ip}" : "No jumpbox created"
}

output "quick_ssh_server" {
  description = "Quick SSH command for server"
  value = length(google_compute_instance.server) > 0 ? "ssh -i ${var.ssh_private_key_path} ${var.ssh_username}@${google_compute_instance.server[0].network_interface[0].access_config[0].nat_ip}" : "No server created"
}
